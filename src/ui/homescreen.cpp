...
uint16_t C_BG, C_HEADER, C_TEXT, C_DIM, C_ACCENT, C_GREEN, C_YELLOW, C_RED,
    C_CARD, C_WHITE;

// UI Cache muuttujat, jotta vältytään välkkymiseltä (mutta voidaan nollata
...
// Sensor-näkymän layout-vakiot
static const int SENSOR_CARD_Y0 = 65; // Ensimmäisen kortin Y-alku
static const int SENSOR_CARD_H = 75;  // Kortin korkeus
static const int SENSOR_CARD_GAP = 8; // Korttien väli
static const int SENSOR_CONTENT_OFFSET =
    35; // Arvo-tekstin offset kortin yläreunasta

// Korttien Y-positiot (lasketaan drawSensorScreen():ssa, käytetään
// update-funktioissa) 0=Valoisuus, 1=BME280, 2=MQ-135, 3=Pöly, 4=Mikrofoni
static int sCardY[5] = {65, 148, 231, 314,
                        397}; // Oletusarvot ennen ensimmäistä piirtoa
...
// Apufunktio ääkkösten (pisteiden) piirtämiseen TFT-näytöllä perusfontin
// vapaaseen tilaan
...
uint16_t routeColor(const char *r) {
  int n = atoi(r);
  if (n == 3)
    return gfx.color565(220, 50, 50);
  if (n == 9)
    return gfx.color565(50, 180, 220);
  if (n == 13)
    return gfx.color565(50, 200, 100);
  if (n == 16)
    return gfx.color565(255, 150, 0);
  if (n == 25)
    return gfx.color565(180, 80, 200);
  if (n == 30)
    return gfx.color565(255, 200, 0);
  return C_ACCENT;
}
...
void drawWifiScreen(const char *status, int progress, uint16_t color) {
  ...
  gfx.setCursor((gfx.width() - sw) / 2, 220);
  gfx.print(status);
...
void drawHomeScreen(float lux) {
  ...
  snprintf(dateBuf, sizeof(dateBuf), "%s %d.%d.%d", weekdays[ti.tm_wday],
...
void updateVUMeter(int volume) {
  ...
  // Skaalaus: Äänitaso voi vaihdella hiljaisesta kohinasta (~10) kovaan
  // puheeseen (~1500)
...
void drawSensorScreen(float lux, int volume, int mqLevel, float dustDensity) {
  ...
  int cardH = SENSOR_CARD_H;
  int spacing = SENSOR_CARD_GAP;

  // Lasketaan korttien Y-positiot dynaamisesti (update-funktiot käyttävät
  // sCardY[])
  for (int i = 0; i < 5; i++) {
    sCardY[i] = SENSOR_CARD_Y0 + i * (cardH + spacing);
  }
  int y = sCardY[0];
...
void updateMQBar(int mqLevel) {
  int barX = 35;
  int barY = sCardY[2] + SENSOR_CONTENT_OFFSET;
  int barW = gfx.width() - 70;
  int barH = 20;

  // Piirretään kehys (vain kerran, jos mahdollista, mutta pidetään tässä
  // yksinkertaisuuden vuoksi) Piirretään se välimuistin ohi vain kerran!
  if (cacheLastMqFillW == -1)
...
void updateSensorVUMeter(int volume) {
  ...
  int barX = 30;
  int barY = sCardY[4] + SENSOR_CONTENT_OFFSET;
  int w = 8;
  int h = 30;
  int spacing = 2;
  int maxBars = 26;
...
void drawInfoScreen() {
  ...
  gfx.setTextColor(C_DIM);
  gfx.setTextSize(2);
  gfx.setCursor(40, y + 20);
  gfx.print("Osoite (Remote View):");
  ...
  gfx.setTextColor(C_DIM);
  gfx.setTextSize(2);
  gfx.setCursor(40, y + 100);
  gfx.print("Verkko:");
  gfx.setTextColor(C_WHITE);
  gfx.setCursor(40, y + 130);
  gfx.print(WiFi.SSID());
...
void updateSensorLuxText(float lux) {
  if (abs(lux - cacheLastLux) > 0.1f || cacheLastLux == -999.0f) {
    cacheLastLux = lux;
    int y = sCardY[0]; // Ensimmäisen kortin Y
...
void updateDustSensorText(float dustDensity) {
  if (abs(dustDensity - cacheLastDust) > 0.5f || cacheLastDust == -999.0f) {
    ...
    int y = sCardY[3] + SENSOR_CONTENT_OFFSET;
    gfx.fillRect(30, y, gfx.width() - 60, 25,
                 C_CARD); // Pyyhi vanha teksti kokonaan (25px riittää)

    // Värikoodaus hiukkasten määrän mukaan (PM2.5)
    uint16_t color = C_GREEN;
    if (dustDensity > 35.0f)
      color = C_YELLOW;
    if (dustDensity > 75.0f)
      color = C_RED;
    if (dustDensity > 150.0f)
      color = gfx.color565(128, 0, 128); // Purppura/vaara
...
void updateSensorBMEText(float temp, float hum, float pres) {
  if (abs(temp - cacheLastTemp) > 0.5f || abs(hum - cacheLastHum) > 1.0f ||
      abs(pres - cacheLastPres) > 1.0f || cacheLastTemp == -999.0f) {
    cacheLastTemp = temp;
    cacheLastHum = hum;
    cacheLastPres = pres;

    int y = sCardY[1] + SENSOR_CONTENT_OFFSET;
